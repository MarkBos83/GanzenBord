...
public class GooseTile extends Squares {
    ...
    public void execute(int i, Goose goose, Board board, int dice1, int dice2, ArrayList<Goose> geese) {
        int newPosition;
        if (goose.back) {
            System.out.println("Je komt op " + goose.position + ", een gans, ga nogmaals het aantal gegooide ogen terug");
            newPosition = goose.position - dice1 - dice2;
        } else {
            System.out.println("Je komt op " + goose.position + ", een gans, ga nogmaals het aantal gegooide ogen verder");
            newPosition = goose.position + dice1 + dice2;
            if (newPosition > 63) {
